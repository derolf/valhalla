...
inline bool UnidirectionalAStar<expansion_direction, FORWARD>::ExpandInner(
  ...
  // skip if not accesible
  if (!((FORWARD ? meta.edge->forwardaccess() : meta.edge->reverseaccess()) & access_mode_)) {
    return false;
  }
  ...
  if (!FORWARD) {
    ...
    auto opp_edge_id = graphreader.GetOpposingEdgeId(meta.edge_id, opp_tile);
    if (!opp_edge_id) {
      return false;
    }
    opp_edge = opp_tile->directededge(opp_edge_id);
  }

  auto real_edge = FORWARD ? meta.edge : opp_edge;
  auto real_edgeid = FORWARD ? meta.edge_id : opp_edge_id;
  ...
  auto is_dest = !!destinations_.count(meta.edge_id);
  if (FORWARD) {
    ...
    if (!costing_->AllowedReverse(meta.edge, pred, opp_edge, opp_tile, opp_edge_id,
  ...
  auto edge_cost = FORWARD ? costing_->EdgeCost(meta.edge, tile, time_info, flow_sources)
                           : costing_->EdgeCost(opp_edge, opp_tile, time_info, flow_sources);
  ...
  // Find destination edges and enqueue
  if (is_dest) {
    for (const auto& dest_path_edge : destination.correlation().edges()) {
      if (dest_path_edge.graph_id() == real_edgeid) {
        auto percent_traversed =
            FORWARD ? dest_path_edge.percent_along() : 1.0 - dest_path_edge.percent_along();

        auto cost = pred.cost() + transition_cost + edge_cost * percent_traversed;

        // We need to penalize this location based on its score (distance in meters from
        // input) We assume the slowest speed you could travel to cover that distance to
        // start/end the route
        // TODO: assumes 1m/s which is a maximum penalty this could vary per costing model
        // Perhaps need to adjust score?
        cost.cost += dest_path_edge.distance();

        // Compute sortcost (distance to target is 0)
        auto sortcost = cost.cost + astarheuristic_.Get(0);

        auto path_distance = static_cast<uint32_t>(pred.path_distance() +
                                                   real_edge->length() * percent_traversed + .5f);

        // Add EdgeLabel to the adjacency list
        auto idx = edgelabels_.size();
        EdgeLabel edge_label(kInvalidLabel, meta.edge_id, meta.edge, cost, sortcost, 0, mode_,
                             path_distance, Cost{}, restriction_idx,
                             pred.closure_pruning() || !(costing_->IsClosed(real_edge, tile)),
                             !!(flow_sources & kDefaultFlowMask),
        ...
        // DO NOT SET EdgeStatus - this is a "virtual" edge that never gets expanded
        ...
        if (best_path.first == -1 || cost.cost < best_path.second) {
          best_path.first = idx;
          best_path.second = cost.cost;
        }

        // keep going, might have multiple correlations on this edge
      }
    }
  }

  auto cost = pred.cost() + transition_cost + edge_cost;

  auto meta_endtile = graphreader.GetGraphTile(meta.edge->endnode());

  // Compute sortcost by appyling heuristic

  float dist = 0.0f;
  auto sortcost =
      cost.cost + astarheuristic_.Get(meta_endtile->get_node_ll(meta.edge->endnode()), dist);

  auto path_distance = static_cast<uint32_t>(pred.path_distance() + real_edge->length() + .5f);

  // Add EdgeLabel to the adjacency list
  auto idx = edgelabels_.size();
  EdgeLabel edge_label(kInvalidLabel, meta.edge_id, meta.edge, cost, sortcost, dist, mode_,
                       path_distance, Cost{}, restriction_idx,
                       pred.closure_pruning() || !(costing_->IsClosed(real_edge, tile)),
                       !!(flow_sources & kDefaultFlowMask),
                       costing_->TurnType(pred.opp_local_idx(), nodeinfo, meta.edge));
...
void UnidirectionalAStar<expansion_direction, FORWARD>::SetOrigin(
  ...
  auto super_trivial = [this](const valhalla::PathEdge& edge) {
    auto edgeid = edge.graph_id();
    if (!destinations_.count(edgeid)) {
      return false;
    }
    for (const auto& destination_edge : destination.correlation().edges()) {
      if (destination_edge.graph_id() == edgeid &&
          edge.percent_along() == destination_edge.percent_along()) {
        return true;
      }
    }
    return false;
  ...
  for (const auto& edge : origin.correlation().edges()) {
    ...
    const auto label_endtile = FORWARD ? endtile : tile;
    const auto label_edgeid = FORWARD ? edgeid : graphreader.GetOpposingEdgeId(edgeid, endtile);
    const auto label_directededge = FORWARD ? directededge : endtile->directededge(label_edgeid);

    uint8_t flow_sources;
    auto edge_cost = costing_->EdgeCost(directededge, tile, time_info, flow_sources);

    // is edge also a destination?
    if (destinations_.count(edgeid)) {
      // Find destination edges and enqueue
      for (const auto& dest_path_edge : destination.correlation().edges()) {
        if (dest_path_edge.graph_id() == edgeid) {
          auto percent_traversed = FORWARD ? (dest_path_edge.percent_along() - percent_along)
                                           : (percent_along - dest_path_edge.percent_along());
          ...
          auto cost = edgecost * percent_traversed;
          ...
          // TODO: assumes 1m/s which is a maximum penalty this could vary per costing model
          // Perhaps need to adjust score?
          cost.cost += edge.distance() + dest_path_edge.distance();
          ...
          // Add EdgeLabel to the adjacency list
          auto idx = edgelabels_.size();
          EdgeLabel edge_label(kInvalidLabel, label_edgeid, label_directededge, cost, sortcost, 0,
                               mode_, path_distance, Cost{}, kInvalidRestriction,
                               !(costing_->IsClosed(directededge, tile)),
                               !!(flow_sources & kDefaultFlowMask), sif::InternalTurn::kNoTurn, );
    ...
    auto percent_traversed = FORWARD ? (1.0f - percent_along) : percent_along;
    ...
    // Compute sortcost by appyling heuristic
    auto sortcost =
        cost.cost + astarheuristic_.Get(label_endtile->get_node_ll(label_directededge->endnode()));

    auto path_distance = static_cast<uint32_t>(directededge->length() * percent_traversed + .5f);

    // Add EdgeLabel to the adjacency list
    auto idx = edgelabels_.size();
    EdgeLabel edge_label(kInvalidLabel, label_edgeid, label_directededge, cost, sortcost, 0, mode_,
                         path_distance, Cost{}, kInvalidRestriction,
                         !(costing_->IsClosed(directededge, tile)),
                         !!(flow_sources & kDefaultFlowMask), sif::InternalTurn::kNoTurn, );
...
UnidirectionalAStar<expansion_direction, FORWARD>::SetDestination(GraphReader& graphreader,
  ...
  for (const auto& edge : dest.correlation().edges()) {
    ...
    // Disallow any user avoided edges if the avoid location is behind the destination along the
    // edge (check has to be done BEFORE we invert the edge if !FORWARD)
    if (FORWARD ? costing_->AvoidAsDestinationEdge(edgeid, percent_along)
                : costing_->AvoidAsOriginEdge(edgeid, percent_along)) {
      continue;
    }
    ...
    if (FORWARD) {
      ...
      // just get the inverted tile because this is where we start to search from
      graphreader.GetOpposingEdgeId(edgeid, tile);
    }
